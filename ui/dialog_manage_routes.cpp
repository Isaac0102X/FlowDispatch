...
DialogManageRoutes::DialogManageRoutes(QWidget *parent)
    ...
    qsValue += QStringLiteral("prefer_ipv4 prefer_ipv6 ipv4_only ipv6_only")
                   .split(" ");
    ui->dns_object->setPlaceholderText(DecodeB64IfValid(
        "ewogICJzZXJ2ZXJzIjogW10sCiAgInJ1bGVzIjogW10sCiAgImZpbmFsIjogIiIsCiAgIn"
        "N0cmF0ZWd5IjogIiIsCiAgImRpc2FibGVfY2FjaGUiOiBmYWxzZSwKICAiZGlzYWJsZV9l"
        "eHBpcmUiOiBmYWxzZSwKICAiaW5kZXBlbmRlbnRfY2FjaGUiOiBmYWxzZSwKICAicmV2ZX"
        "JzZV9tYXBwaW5nIjogZmFsc2UsCiAgImZha2VpcCI6IHt9Cn0="));
    ...
    //
    const auto sourceStringsDomain =
        Qv2ray::components::GeositeReader::ReadGeoSiteFromFile(geositeFn);
    directDomainTxt =
        new AutoCompleteTextEdit("geosite", sourceStringsDomain, this);
    proxyDomainTxt =
        new AutoCompleteTextEdit("geosite", sourceStringsDomain, this);
    blockDomainTxt =
        new AutoCompleteTextEdit("geosite", sourceStringsDomain, this);
    //
    const auto sourceStringsIP =
        Qv2ray::components::GeositeReader::ReadGeoSiteFromFile(geoipFn);
    ...
    //
    REFRESH_ACTIVE_ROUTING(
        NekoGui::dataStore->active_routing, NekoGui::dataStore->routing.get())
...
void DialogManageRoutes::accept()
    ...
    NekoGui::dataStore->routing->fn =
        ROUTES_PREFIX + NekoGui::dataStore->active_routing;
...
QList<QAction *> DialogManageRoutes::getBuiltInSchemes()
    ...
    QList<QAction *> list;
    list.append(
        this->schemeToAction(tr("Bypass LAN and China"), routing_cn_lan));
    list.append(this->schemeToAction(tr("Global"), routing_global));
    return list;
    }

QAction *DialogManageRoutes::schemeToAction(
    const QString &name, const NekoGui::Routing &scheme)
    {
...
void DialogManageRoutes::UpdateDisplayRouting(NekoGui::Routing *conf, bool qv)
    ...
    ui->outbound_domain_strategy->setCurrentText(
        conf->outbound_domain_strategy);
...
void DialogManageRoutes::SaveDisplayRouting(NekoGui::Routing *conf)
    ...
    conf->outbound_domain_strategy =
        ui->outbound_domain_strategy->currentText();
...
void DialogManageRoutes::on_load_save_clicked()
    ...
    connect(
        list, &QListWidget::currentTextChanged, lineEdit, &QLineEdit::setText);
    ...
    connect(load, &QPushButton::clicked, w, [=]
    {
        ...
        if (!fn.isEmpty())
            ...
            if (r->Load())
                {
                if (QMessageBox::question(nullptr, software_name,
                        tr("Load routing: %1").arg(fn) + "\n" +
                            r->DisplayRouting()) == QMessageBox::Yes)
                    {
                    REFRESH_ACTIVE_ROUTING(
                        fn, r.get()) // temp save to the window
                    w->accept();
                    }
                }
            }
    ...
    connect(remove, &QPushButton::clicked, w, [=]
    {
        ...
        if (!fn.isEmpty() && NekoGui::Routing::List().length() > 1)
            {
            if (QMessageBox::question(nullptr, software_name,
                    tr("Remove routing: %1").arg(fn)) == QMessageBox::Yes)
                {
                QFile f(ROUTES_PREFIX + fn);
                f.remove();
                if (NekoGui::dataStore->active_routing == fn)
                    {
                    NekoGui::Routing::SetToActive(
                        NekoGui::Routing::List().first());
                    REFRESH_ACTIVE_ROUTING(NekoGui::dataStore->active_routing,
